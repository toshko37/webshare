...
echo -e "${CYAN}╚╩╝└─┘└─┘╚═╝┴ ┴┴ ┴┴└─└─┘${NC}"
echo ""
echo -e "${BLUE}Installer v3.2${NC} - Source: ${GREEN}$SOURCE${NC}"
echo ""
...
check_existing_installation() {
    ...
    # Check for index.php
    if [ -f "$INSTALL_DIR/src/index.php" ]; then
        has_existing=true
...
# Check for existing vhost configs that might have wrong paths
VHOST_UPDATED=false
for vhost_file in /etc/apache2/sites-available/*.conf /etc/apache2/sites-enabled/*.conf; do
    [ -f "$vhost_file" ] || continue
    if grep -q "ServerName.*${DOMAIN}" "$vhost_file" 2>/dev/null; then
        # Check if DocumentRoot is wrong
        OLD_DOCROOT=$(grep -oP 'DocumentRoot\s+\K[^\s]+' "$vhost_file" 2>/dev/null | head -1)
        if [ -n "$OLD_DOCROOT" ] && [ "$OLD_DOCROOT" != "$SRC_DIR" ]; then
            warn "Found vhost with wrong DocumentRoot: $OLD_DOCROOT"
            warn "Updating to: $SRC_DIR"
            # Fix DocumentRoot
            sed -i "s|DocumentRoot.*|DocumentRoot ${SRC_DIR}|g" "$vhost_file"
            # Fix Directory directives
            sed -i "s|<Directory [^>]*webshare[^>]*>|<Directory ${SRC_DIR}/>|g" "$vhost_file"
            VHOST_UPDATED=true
        fi
    fi
done

if [ "$VHOST_UPDATED" = true ]; then
    success "Existing vhost paths updated"
else
    # Create new vhost config
    cat > "/etc/apache2/sites-available/${DOMAIN}.conf" << VHOST
# WebShare - ${DOMAIN}
# Generated by WebShare Installer v3.2
